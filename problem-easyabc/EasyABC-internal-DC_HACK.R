...
.ABC_rejection_internal_cluster <- function(model, prior, prior_test, nb_simul, seed_count = 0, 
    n_cluster = 1) {
    cl <- makeCluster(getOption("cl.cores", n_cluster)) ; ex <- Filter(function(x) is.function(get(x, .GlobalEnv)), ls(.GlobalEnv)) ; clusterExport(cl, ex) ; ex <- Filter(function(x) is.function(get(x, .GlobalEnv)), ls(.GlobalEnv)) ; clusterExport(cl, ex)
    tab_simul_summarystat = NULL
    tab_param = NULL
...
# from ".ABC_rejection_cluster"
# to "dc_ABC_rejection_cluster"

dc_ABC_rejection_cluster <- function(model, prior, prior_test, nb_simul, seed_count = 0, 
    n_cluster = 1, verbose) {
    if (verbose) {
        write.table(NULL, file = "output", row.names = F, col.names = F, quote = F)
    }
...
.ABC_launcher_not_uniform_cluster <- function(model, prior, param_previous_step, 
    ...
    tab_simul_summarystat = NULL
    tab_param = NULL
    cl <- makeCluster(getOption("cl.cores", n_cluster)) ; ex <- Filter(function(x) is.function(get(x, .GlobalEnv)), ls(.GlobalEnv)) ; clusterExport(cl, ex)
    list_param = list(NULL)
    npar = floor(nb_simul/(100 * n_cluster))
    n_end = nb_simul - (npar * 100 * n_cluster)
    if (npar > 0) {
